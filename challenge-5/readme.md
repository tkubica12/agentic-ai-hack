...
1. Open `challenge-5/deployment/local-test.sh` file and replace `RESOURCE_GROUP` value with your resource group name. Change `SP_NAME` variable to some unique name. **Run the helper script** for automated setup and testing:
```bash
...
3. **Deploy to Azure**: Now it's time to push it to the Cloud! Rename your file from `container-apps copy.sh` to `container-apps.sh`, fill the cells that have the #FILL comment, copy-paste variables from `.env` to `--env-vars` section and run:
```bash
./container-apps.sh
```
